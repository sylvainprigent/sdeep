...
class ClassifierResBlock(torch.nn.Module):
    def __init__(self,
        ...
        for bloc in range(n_blocs):
            ...
            self._seq.append(torch.nn.ReLU())  # TODO remove relu for last block ?
...
class Classifier(torch.nn.Module):
    def __init__(self,
        ...
        self._features.append(torch.nn.Conv2d(n_channels_in, blocs_channels[0], kernel_size=3, padding=1))
        ...
        for idx in range(len(blocs_channels)):
            ...
            for _ in range(n_res_per_bloc):
                self._features.append(ClassifierResBlock(nc_in, n_conv_per_res_bloc, use_batch_norm))
            self._features.append(torch.nn.Conv2d(nc_in, nc_out, kernel_size=2, stride=2, padding=0,
                                  bias=False))
