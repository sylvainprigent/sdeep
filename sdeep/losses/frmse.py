"""Fourier ring correlation loss"""
import numpy as np
import torch
from skimage import draw


class FRMSELoss(torch.nn.Module):
    """Define an image reconstruction loss with the Fourier Ring MSE
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        """forward method from torch module"""
        ...
        for radius in range(1, r_max - 1):
            ...
            curve_[:, :, radius] = torch.sum(
                torch.square(torch.abs(p_1 - p_2)))
